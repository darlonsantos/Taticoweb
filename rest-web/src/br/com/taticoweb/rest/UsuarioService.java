package br.com.taticoweb.rest;

import java.util.ArrayList;
import java.util.List;

import javax.annotation.PostConstruct;
import javax.ws.rs.Consumes;
import javax.ws.rs.FormParam;
import javax.ws.rs.GET;
import javax.ws.rs.POST;
import javax.ws.rs.Path;
import javax.ws.rs.Produces;
import javax.ws.rs.core.MediaType;

import br.com.taticoweb.dao.UsuarioDAO;
import br.com.taticoweb.entity.Profissao;
import br.com.taticoweb.entity.User;

@Path("/user")
public class UsuarioService {

	private UsuarioDAO usuarioDAO;

	@PostConstruct
	private void init() {
		usuarioDAO = new UsuarioDAO();
	}

	@POST
	@Path("/login")
	@Produces(MediaType.TEXT_PLAIN)
	public boolean isLoggedIn(@FormParam("usuario") String usuario, @FormParam("senha") String senha) {
		try {
			return usuarioDAO.isLoggedIn(usuario, senha);
		} catch (Exception e) {
			e.printStackTrace();
			return false;
		}
	}

	@POST
	@Path("/add")
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.APPLICATION_JSON)
	public User add(User user) {
		return user;
	}

	@GET
	@Path("/get_profissoes")
	@Produces(MediaType.APPLICATION_JSON)
	public List<Profissao> listProfissoes() {
		List<Profissao> profissoes = new ArrayList<>();
		
		Profissao profissao = new Profissao();
		profissao.setCodProfissao(1);
		profissao.setDescricao("M�dico");
		profissao.setSubDescricao("Otorrinolaringologista");
		profissao.setUrlImg("img/doctor.png");
		profissoes.add(profissao);
		
		profissao = new Profissao();
		profissao.setCodProfissao(2);
		profissao.setDescricao("Analista Financeiro");
		profissao.setSubDescricao("Especialista em Bolsa");
		profissao.setUrlImg("img/money.png");
		profissoes.add(profissao);
		
		profissao = new Profissao();
		profissao.setCodProfissao(3);
		profissao.setDescricao("Professor");
		profissao.setSubDescricao("Qu�mica e F�sica");
		profissao.setUrlImg("img/teacher.png");
		profissoes.add(profissao);
		
		return profissoes;
	}

}
