...
import br.com.taticoweb.entity.Usuario;
import br.com.taticoweb.entity.Venda;
import br.com.taticoweb.util.PagtoCliente;
...
public class CartService {
	...
	public Response checkPagto(@FormParam("idPagto") String idPagto, //
		...
		br.com.taticoweb.util.Response response = new br.com.taticoweb.util.Response();
		Gson gson = new Gson();
		...
		try {
			...
			if (Double.parseDouble(quantiaServidor) != quantiaCliente) {
				response.setErro(true);
				response.setMsg("Valores de quantias n�o conferem!");
				return Response.ok(gson.toJson(response), MediaType.APPLICATION_JSON).build();
			}
			
			if (!moedaServidor.equals(moedaCliente)) {
				response.setErro(true);
				response.setMsg("Moedas n�o conferem!");
				return Response.ok(gson.toJson(response), MediaType.APPLICATION_JSON).build();
			}
			
			if (!STATUS_COMPLETED.equals(estadoVenda)) {
				response.setErro(true);
				response.setMsg("Venda n�o completada!");
				return Response.ok(gson.toJson(response), MediaType.APPLICATION_JSON).build();
			}
			...
			e.printStackTrace();
			response.setErro(true);
			response.setMsg("Houve um erro na verifica��o do pagamento junto ao PayPal. Erro: " + e.getMessage());
			return Response.ok(gson.toJson(response), MediaType.APPLICATION_JSON).build();
		}

		response.setErro(false);
		response.setMsg("Verifica��o de pagamento efetuada com sucesso!");
		return Response.ok(gson.toJson(response), MediaType.APPLICATION_JSON).build();
	}
	...
	private void inserirItensVenda(Pagamento pagtoFinal, Transaction transaction, String estadoVenda) throws Exception {
		...
		for (Item i : lista.getItems()) {
			...
			Produto produto = produtoDAO.buscarProdutoPorSku(i.getSku());
